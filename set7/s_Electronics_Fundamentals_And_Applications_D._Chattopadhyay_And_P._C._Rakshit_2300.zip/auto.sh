...
SCI_PATH="/home/fossee/Downloads/scilab-5.5.2/bin/scilab-adv-cli"
...
CAT_FILE=$(egrep -r "subplot\S[0-9]?[d]?[0-9]?[(][)]*|clf\S[0-9]?[d]?[0-9]?[(][)]|plot\S[0-9]?[d]?[0-9]?[(][)]|gca\S[0-9]?[d]?[0-9]?[(][)]|gcf\S[0-9]?[d]?[0-9]?[(][)]*|figure|plot|plot2d|plot3d" ${sce_file})
	...
	then
		...
		echo "grepthis#${sce_file} #" >> ./output_${ZIPFILE}.log
		...
		echo "grepthis#${sce_file} #" >> ./output_graph_${ZIPFILE}.log

...
Sum = sum(y);
// Since Sum is sufficiently  to 1.0, therefore:
q = 0.67;// [LF_By_F]
// Assume:
// C3: light key
// C5: heavy key
...
// Temp_avg is very  to the assumed 80 OC
Rm = (Val/D)-1;
...
D = sum(yD2);// [kmol]
W = sum(xW2);// [kmol]
// The distillate dew point computes to 46.6 OC and the residue bubble point computes to 113 OC, which is significantly  to the assumed.
...
// It is  that the conc. of these components are reducing so rapidly that there is no need to go an further.
